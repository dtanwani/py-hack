...
- 🎯 Entry points to execute your program using `python -m <py_hack>` or `$ py_hack` with basic CLI argument parsing.
...
---
# py_hack Flask Application

Awesome py_hack created by dtanwani
...
```bash
git clone https://github.com/dtanwani/py-hack py_hack
cd py_hack
make install
```
...
```bash
py_hack create-db   # run once
py_hack populate-db  # run once (optional)
py_hack add-user -u admin -p 1234  # ads a user
py_hack run
```
