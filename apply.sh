...
original_name="py_hack"
original_urlname="py-hack"
original_description="Awesome py_hack created by dtanwani" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/py_hack "${name}"
...
then
    ...
    echo "then run 'py_hack' or 'python -m py_hack'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'py_hack' or 'python -m py_hack' or 'poetry run py_hack'"
fi
